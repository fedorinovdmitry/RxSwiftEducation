...
//  RxSwiftEducation
//
//  Created by Дмитрий Федоринов on 23.03.2022.
//
...
//                return request
//            }
//            .flatMap { request -> Observable<(response: HTTPURLResponse,
//                                              data: Data)> in
        ...
        let response = Observable.from(["https://api.github.com/search/repositories?q=language:swift&per_page=5"])
            ...
            .flatMap { list -> Observable<[String]> in
                guard let list = list as? [String: Any],
                      let itemsList = list["items"] as? [[String: Any]]
                 else { return Observable.never() }
                let names = itemsList.compactMap { $0["full_name"] as? String }
                return Observable.just(names)
            }
            .flatMap { names -> Observable<String> in
                return Observable<String>.from(names)
            }
...
extension ActivityController {
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.detailTextLabel?.text = event.repo.name + ", " +
                                     event.action.replacingOccurrences(of: "Event", with: "").lowercased()
